...
int main(void) {
    ...
    printf("\n\nWelcome to xzhan214's sort.c, compiled on %s %s.\n", __DATE__, __TIME__);
...
ListItem *SelectionSort(ListItem* list) {
    ...
    ListItem* i;
    ListItem* j;
    for (i = head; i != NULL; i = i->nextItem) {
        ListItem* current = i;
        for (j = i->nextItem; j != NULL; j = j->nextItem) {
            ListItem* cur = current;
            ListItem* now = j;
            if (strcmp(now->data, cur->data) < 0) {
                current = j;
            }
        }
        if (current != i) {
            LinkedListSwapData(current, i);
        }
    }
...
ListItem *InsertionSort(ListItem* list) {
    ListItem *head = LinkedListGetFirst(list);
    ListItem *i;
    for (i = head->nextItem; i != NULL; i = i->nextItem) {
        char * data = i->data;
        ListItem * j = i->previousItem;
        while (j != NULL && strcmp(j->data, data) > 0) {
            LinkedListSwapData(j, j->nextItem);
            j = j->previousItem;
        }
        if (j != NULL) {
            j->nextItem->data = data;
        }
    }
